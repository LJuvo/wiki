...
ThBox.prototype.init = (boxId, boxWidth, boxHeight) => {
    ...
    var texture1 = new THREE.TextureLoader().load("a.gif");
    var texture2 = new THREE.TextureLoader().load("a.gif");

    // 对于 THREE.Points，由于其一个实例只是设置一种纹理，因此对于多个纹理，需要创建相应个数的 THREE.Points
    var points1 = createPoints("aotu", texture1, 20, true, 0.8, true, 0xffffff);
    var points2 = createPoints("joy", texture2, 20, true, 0.8, true, 0xffffff);
        ...
        scene.children.forEach(function(child) {
            if (child instanceof THREE.Points) {
                ...
                vertices.forEach(function(v) {
                    ...
                    if (v.y <= -boxHeight / 2) v.y = boxHeight / 2;
                    if (v.x <= -boxWidth / 2 || v.x >= boxWidth / 2)
